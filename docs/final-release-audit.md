...
- La navegación exacta desde Items a un nivel de Corporation se retiró posteriormente por decisión de producto, junto con su código y pruebas específicas. Items conserva referencias compactas no interactivas con el primer término del nombre y el nivel.
...
| Items `/items`               | Filtros, búsqueda vacía, tabla virtual de 104 items, iconos, referencias de Corporation y acción Planner; escritorio y móvil                                          | Sin incidencia confirmada. En móvil la tabla ancha tiene scroll interno operable; el resto de columnas no aparece en el primer encuadre, una limitación deliberada del patrón de tabla densa. |
...
| Corporations `/corporations` | Cinco corporaciones, 72 niveles, acordeón abierto y recompensas; 320 px                                                                                               | Sin recorte documental ni incidencia confirmada.                                                                                                                                              |
...
- Tareas: ejecutar gate completo; comprobar en el hosting actual rutas directas, recarga, 404, favicon, recursos, consola y red; repetir portada móvil/escritorio y al menos un plan denso; probar Firefox si se dispone de su binario; registrar métricas de campo solo si existen.
...
- Pruebas: `pnpm test:all`, Lighthouse dirigido, navegador manual en los estados corregidos, `git diff --check` y `git status`. **Estado:** gate local y revisión del despliegue existente completados; comparación posterior al despliegue y validación funcional en Firefox pendientes. `pnpm test:all` pasó de nuevo el 21/09/2026: catálogo de skills, formato, lint, tipos E2E, build, 82 tests unitarios y 21 E2E de Chromium; 103 tests aprobados, ninguno fallido u omitido. El build servido con `pnpm preview` resolvió las cinco rutas y el 404, sin errores de consola ni peticiones fallidas; favicon y recursos cargaron con 200.
