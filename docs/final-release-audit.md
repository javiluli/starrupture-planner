...
- Pruebas: `pnpm test:all`, Lighthouse dirigido, navegador manual en los estados corregidos, `git diff --check` y `git status`. **Estado:** verificación local completada; host público y Firefox pendientes. El build servido con `pnpm preview` resolvió las cinco rutas y el 404, sin errores de consola ni peticiones fallidas; favicon y recursos cargaron con 200. No se desplegó.

**Siguiente alcance pendiente:** comprobar el host público después de publicar y Firefox si se dispone de su binario. No se requiere otro cambio local confirmado.
